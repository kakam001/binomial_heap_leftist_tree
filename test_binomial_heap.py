...
SAMPLES = [17, 73, 48, 2, 91, 33, 59, 84, 12, 67, 5, 27]  # 3 more params than leftist for func
VAL_ONE = 25
VAL_TWO = 47
NEW_VAL = 2
...
class TestBinomialHeap:
    def __init__(self):
        self.test_binomial_heap = BinomialHeap()
    ...
    def test_make_heap(self):
        ...
        self.test_binomial_heap.make_heap()
    ...
    def test_minimum(self):
        self.reset_times()
        self.start_time = timer()
        self.test_binomial_heap.minimum()
        self.stop_time = timer()
    ...
    def test_full_insert(self, seed=42):
        ...
        for sample in samples:
            self.test_binomial_heap.insert(sample)
        self.stop_time = timer()
        del samples
        return self.calc_time()
    ...
    def test_extract_min(self):
        self.reset_times()
        self.start_time = timer()
        self.test_binomial_heap.extract_min()
        self.stop_time = timer()
    ...
    def test_single_insert(self):
        ...
        self.reset_times()
        self.start_time = timer()
        self.node_to_delete = self.test_binomial_heap.insert(sample)
        self.stop_time = timer()
        return self.calc_time()
    ...
    def test_decrease_key(self):
        ...
        if self.node_to_delete is not None:
            self.reset_times()
            self.start_time = timer()
            self.test_binomial_heap.decrease_key(self.node_to_delete, new_val)
            self.stop_time = timer()
            self.node_to_delete = None
    ...
    def test_delete(self):
        ...
        if self.node_to_delete is not None:
            self.reset_times()
            self.start_time = timer()
            self.test_binomial_heap.delete(self.node_to_delete)
            self.stop_time = timer()
            self.node_to_delete = None
    ...
    def functional_test(self):
        ...
        # MINIMUM
        print("\nMINIMUM\n")
        mini = self.test_binomial_heap.minimum()
        ...
        node = self.test_binomial_heap.insert(VAL_ONE)
        self.print_tree()
        print(f"\nthen delete insert {VAL_ONE}\n")
        self.test_binomial_heap.delete(node)
        self.print_tree()

        # DECREASE_KEY (insert needed)
        print("\nDECREASE_KEY\n")
        print(f"first insert {VAL_TWO}\n")
        node = self.test_binomial_heap.insert(VAL_TWO)
        self.print_tree()
        print(f"\nthen decrease the key of {VAL_TWO} to the new value {NEW_VAL}\n")
        self.test_binomial_heap.decrease_key(node, NEW_VAL)
        self.print_tree()
